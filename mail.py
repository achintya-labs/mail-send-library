...
def get_credentials():
    ...
    working_dir = os.path.expanduser('.') #>> C:\Users\Me
    credential_dir = os.path.join(working_dir, 'credentials') # >>C:\Users\Me\.credentials   (it's a folder)
    if not os.path.exists(credential_dir):
        os.makedirs(credential_dir)  #create folder if doesnt exist
    credential_path = os.path.join(credential_dir, CRED)
...
def create_message_and_send(sender, to, subject,  message_text_plain="", message_text_html="", attached_file="", cc=""):
    ...
    if attached_file=="":
        ...
        send_Message_without_attachment(service, "me", message_without_attachment)
    else:
        message_with_attachment = create_Message_with_attachment(sender, to, cc, subject, message_text_plain, message_text_html, attached_file)
        send_Message_with_attachment(service, "me", message_with_attachment, attached_file)
...
def send_Message_without_attachment(service, user_id, body):
    try:
...
def send_Message_with_attachment(service, user_id, message_with_attachment, attached_file):
